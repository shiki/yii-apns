 ...
 * @author Shiki
 */
class SAPNS extends CApplicationComponent
{
  ...
  public function init()
  {
    ...
    $this->initAutoloader();
    require_once(dirname(__FILE__) . '/SAPNSLogger.php');
  ...
  public function getPushProvider()
  {
    ...
    $push->setLogger(new SAPNSLogger());
    $push->connect();
  ...
  {
    defined('SAPNS_LIB_PATH') 
      or define('SAPNS_LIB_PATH', $this->apnsPHPLibPath ? $this->apnsPHPLibPath : dirname(__FILE__));
   ...
   * 
   * This autoloader fails if initAutoloader is not called first or SAPNS_LIB_PATH is not defined.
   * 
   * @param string $className 
   */
  public static function autoload($className)
  {
    if (!defined('SAPNS_LIB_PATH'))
      return;
    if (empty($className) || strpos($className, 'ApnsPHP_') !== 0)
      return;

    $filePath = sprintf('%s%s%s.php',
      SAPNS_LIB_PATH, DIRECTORY_SEPARATOR,
